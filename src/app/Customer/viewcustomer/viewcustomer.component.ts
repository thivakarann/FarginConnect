...
import { CreateCampaginComponent } from '../create-campagin/create-campagin.component';
import { BulkSetupboxactiveinactiveComponent } from '../bulk-setupboxactiveinactive/bulk-setupboxactiveinactive/bulk-setupboxactiveinactive.component';
import { CreateSettopboxbulkupdateComponent } from '../create-settopboxbulkupdate/create-settopboxbulkupdate.component';
import { CreateDuestatusbulkComponent } from '../create-duestatusbulk/create-duestatusbulk.component';
...
currentfilval: any;
  ...
  excelexportDetails() {
    ...
    data.forEach((d: any) => {
      ...
      const rowData = [
        ...
        d.branchName || '',
      `"${d.setupBoxNumber}"` || '',
      d.setupBoxDoorNumber || '',
      d.setupBoxAreaName  || '',
      d.setupBoxStreetName || '',
  ...
  excelsetupboxactiveinactive() {
    ...
    // Create a Blob and save as CSV
    const blob = new Blob([csvContent.join('\n')], { type: 'text/csv;charset=utf-8;' });
    FileSaver.saveAs(blob, 'SetupBox Active/Inactive.csv');
  }
 
  createssetupboxactiveinactive() {
    this.dialog.open(BulkSetupboxactiveinactiveComponent,
      {
        disableClose: true,
        enterAnimationDuration: '500ms',
        exitAnimationDuration: '1000ms',
  ...
  excelsetupboxupdate() {
    ...
    data.forEach((d: any) => {
     ...
     `"${d.setupBoxNumber}"` || '',
        Array.isArray(d.channelName) ? d.channelName.join(', ') : d.channelName,
        Array.isArray(d.broadCasterName) ? d.broadCasterName.join(', ') : d.broadCasterName,
        Array.isArray(d.bouquetName) ? d.bouquetName.join(', ') : d.bouquetName,
        Array.isArray(d.planName) ? d.planName.join(', ') : d.planName,
        d.updateType || '',
        // Add any other fields as needed
    ...
    // Create a Blob and save as CSV
    const blob = new Blob([csvContent.join('\n')], { type: 'text/csv;charset=utf-8;' });
    FileSaver.saveAs(blob, 'SetupBox Update.csv');
  }
 
  createssetupboxupdate() {
    this.dialog.open(CreateSettopboxbulkupdateComponent,
      {
        disableClose: true,
        enterAnimationDuration: '500ms',
        exitAnimationDuration: '1000ms',
  ...
  excelduestatus() {
    ...
    // Create a Blob and save as CSV
    const blob = new Blob([csvContent.join('\n')], { type: 'text/csv;charset=utf-8;' });
    FileSaver.saveAs(blob, 'SettopBox Duestatus Update.csv');
  }
  createsduestatusbulk() {
    this.dialog.open(CreateDuestatusbulkComponent,
      {
        disableClose: true,
        enterAnimationDuration: '500ms',
        exitAnimationDuration: '1000ms',
