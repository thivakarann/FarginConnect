...
@Component({
  selector: 'app-view-setupbulkresponse',
  templateUrl: './view-setupbulkresponse.component.html',
  styleUrl: './view-setupbulkresponse.component.css'
})
export class ViewSetupbulkresponseComponent {
  dataSource: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    this.service.customersetupboxbulkupdateResponse(this.merchantId).subscribe((res: any) => {
      if(res.flag==1)
      {
  ...
  responseDownload(uploadId: any) {
    this.service.customersettopbulkruploads(uploadId).subscribe((res: any) => {
  ...
  responseExcel() {
    ...
    let worksheet = workbook.addWorksheet('Setupbox-PlanUpdate-Details');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'Setupbox-PlanUpdate-Details.csv');
    });
  }
      ...
      if (res.flag == 1) {
        ...
        this.responseData?.forEach((element: any) => {
          ...
          this.dataPush.push(element?.setupBoxNumber);
            ...
            this.dataPush.push(element?.alcotChannelNames.join(", "));
            this.dataPush.push(element?.alcotChannelNames.join(", "));
             this.dataPush.push(element?.bouquetNames.join(", "));
          this.dataPush.push(element?.lcopPlanNames.join(", "));
          if (element?.updateType == 0) {
            this.dataPush.push('New Plan')
          }
          else {
            this.dataPush.push('Added Extra Plan ')
          }
  ...
  responseExcels() {
    ...
    const data = this.responseExcelData;
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Setupbox-PlanUpdate-UploadedFile');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'Setupbox-PlanUpdate-UploadedFile.csv');
    });
  }
