...
@Component({
  selector: 'app-view-duebulkresponse',
  templateUrl: './view-duebulkresponse.component.html',
  styleUrl: './view-duebulkresponse.component.css'
})
export class ViewDuebulkresponseComponent {
 dataSource: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    this.service.setupboxduebulkupdateResponse(this.merchantId).subscribe((res: any) => {
      if(res.flag==1)
      {
  ...
  responseDownload(uploadId: any) {
    this.service.settopduebulkruploads(uploadId).subscribe((res: any) => {
    ...
    this.service.settopduebulkruploads(uploadId).subscribe((res: any) => {
      ...
      if (res.flag == 1) {
        ...
        this.responseData?.forEach((element: any) => {
          this.dataPush = [];
          this.dataPush.push(sno);
          this.dataPush.push(element?.customerMobileNumber.join(", "));
          this.dataPush.push(element?.setupBoxNumber);
          this.dataPush.push(element?.dueStatus);
  ...
  responseExcels() {
    ...
    const data = this.responseExcelData;
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Due Status Uploaded File');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'Due Status Uploaded File.csv');
    });
  }
