...
searchPerformed: boolean=false;
  ...
  responseExcel() {
    ...
    let worksheet = workbook.addWorksheet('Customer-Uploaded-Details');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'Customer-Uploaded-Details.csv');
    ...
    this.service.customerbulkbyId(uploadId).subscribe((res: any) => {
      ...
      if (res.flag == 1) {
        ...
        this.responseData?.forEach((element: any) => {
          ...
          this.dataPush.push(element?.alterMobileNumber)
          this.dataPush.push(element?.age)
          this.dataPush.push(element?.emailAddress)
          this.dataPush.push(element?.doorNumber)
          this.dataPush.push(element?.blockNumber)
          this.dataPush.push(element?.flatNumber)
          this.dataPush.push(element?.houseName)
          this.dataPush.push(element?.apartmentName)
          this.dataPush.push(element?.area)
          this.dataPush.push(element?.streetName)
          this.dataPush.push(element.landmark)
         ...
         if (element.billingDate) {
                          this.dataPush.push(moment(element?.billingDate).format('DD/MMM/yyyy').toString());
                        }
                 else {
                          this.dataPush.push('');
          ...
          this.dataPush.push(element?.branchName)
          this.dataPush.push(element?.setupBoxNumber)
          this.dataPush.push(element?.setupBoxDoorNumber)
          this.dataPush.push(element?.setupBoxAreaName)
          this.dataPush.push(element?.setupBoxStreetName)
          const alcotSuccessString = element?.['alcloChannel']?.join(', ') || '';
          const bouquetSuccessString = element?.['bouquetChannel']?.join(', ') || '';
          const lcopSuccessString = element?.['planName']?.join(', ') || '';
 
     
 
          this.dataPush.push(alcotSuccessString);
          this.dataPush.push(bouquetSuccessString);
          this.dataPush.push(lcopSuccessString);
  ...
  responseExcels() {
    ...
    const header = ["S.No",
      ...
      'Branch Name',
      'SetupBoxNumber',
      'Setupbox Door Number',
      'Setupbox Area Name',
      'Setupbox Street Name',
      'Channel Name',
      'Bouquet Name',
      'Plan Name'
      ]
    ...
    const data = this.responseExcelData;
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Customer-UploadedFile');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'Customer-UploadedFile.csv');
