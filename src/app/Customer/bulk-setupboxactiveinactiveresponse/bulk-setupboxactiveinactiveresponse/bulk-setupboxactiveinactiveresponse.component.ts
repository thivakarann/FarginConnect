...
@Component({
  selector: 'app-bulk-setupboxactiveinactiveresponse',
  templateUrl: './bulk-setupboxactiveinactiveresponse.component.html',
  styleUrl: './bulk-setupboxactiveinactiveresponse.component.css'
})
export class BulkSetupboxactiveinactiveresponseComponent {
 dataSource: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    this.service.customersetupboxactiveinactivebulkResponse(this.merchantId).subscribe((res: any) => {
      if(res.flag==1)
      {
  ...
  responseDownload(uploadId: any) {
    this.service.customerbulksetupactiveinactivebyId(uploadId).subscribe((res: any) => {
  ...
  responseExcel() {
    ...
    let worksheet = workbook.addWorksheet('STB-Active-InActive-Details');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'STB-Active-InActive-Details.csv');
      ...
      if (res.flag == 1) {
        ...
        this.responseData?.forEach((element: any) => {
          ...
          if (element?.activeStatus == 1) {
            this.dataPush.push('Active')
          }
          else {
            this.dataPush.push('Inactive')
          }
          sno++;
          this.responseExcelData.push(this.dataPush);
  ...
  responseExcels() {
    ...
    const data = this.responseExcelData;
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('STB-Active-InActive-Uploaded File');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'STB-Active-InActive-Uploaded File.csv');
