...
searchPerformed: boolean=false;
  ...
  responseExcel() {
    ...
    let worksheet = workbook.addWorksheet('Route-Uploaded-Details');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'Route-Uploaded-Details.csv');
    });
  }
    ...
    this.service.routebulkuploadById(uploadId).subscribe((res: any) => {
      ...
      if (res.flag == 1) {
        ...
        this.responseData?.forEach((element: any) => {
          ...
          this.dataPush.push(sno);
          this.dataPush.push(element?.MobileNumber);
          this.dataPush.push(element?.BeatRole.join(", "));
  ...
  responseExcels() {
    const header = ['S.No', 'Employee Mobile Number', 'Beat Role', 'Customer Mobile Number'];
 
    const data = this.responseExcelData;
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Route-UploadedFile');
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, 'Route-UploadedFile.csv');
    });
  }
