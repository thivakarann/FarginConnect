...
import { CreateBulkfiledexectivenameComponent } from '../create-bulkfiledexectivename/create-bulkfiledexectivename.component';
...
export class ViewRouteComponent {
  ...
  displayedColumns: any[] = ["stbId", "adminName", "beatRole","mobileNumber", "status", "action", "View", "viewpincode", "CreatedBy", "createdAt"];
  isChecked: any;
  ...
  openfield() {
    const header = ['mobileNumber', 'merchantAdminMobileNo','updateType'];
    const data = this.responseDataListnew;
    const csvContent = [];
    ...
    const blob = new Blob([csvContent.join('\n')], { type: 'text/csv;charset=utf-8;' });
    FileSaver.saveAs(blob, 'Update Employee.csv');
}
  createroutefield() {
    this.dialog.open(CreateBulkfiledexectivenameComponent,
      {
        disableClose: true,
        enterAnimationDuration: '1000ms',
        exitAnimationDuration: '1000ms',
