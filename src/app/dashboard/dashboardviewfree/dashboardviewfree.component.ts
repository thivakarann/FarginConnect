...
@Component({
  selector: 'app-dashboardviewfree',
  templateUrl: './dashboardviewfree.component.html',
  styleUrl: './dashboardviewfree.component.css'
})
export class DashboardviewfreeComponent {
  dataSource: any;
  merchantId: any = localStorage.getItem('merchantId');
  displayedColumns: any[] = ["stbId","MSO", "Region",  "setupBoxNumber",  "status","bookingStatus"];
  bulkdata: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    this.service.dashboardsetupboxStatusPagination(this.merchantId,0,this.pageSize,this.pageIndex).subscribe((res: any) => {
      if(res.flag==1)
      {
        this.freestatus = res.response.content
        this.dataSource = new MatTableDataSource(this.freestatus.reverse())
  ...
  exportexcel() {
    this.service.dashboardsetupboxStatusExport(this.merchantId,0).subscribe((res: any) => {
 ...
 if (res.flag == 1) {  
    ...
    this.freestatus?.forEach((element: any) => {
      this.response = [];
...
excelexportCustomer() {
  ...
  let worksheet = workbook.addWorksheet('Available Setupbox');
  // Blank Row
  worksheet.addRow([]);
  ...
  workbook.xlsx.writeBuffer().then((data: any) => {
    ...
    FileSaver.saveAs(blob, 'Available Setupbox.xlsx');
  });
}
  ...
  bookedsearch(filterValue: string) {
    if (filterValue) {
    this.service.SetupboxDashboardSearch(this.merchantId,0,filterValue,this.pageSize1,this.pageIndex1).subscribe({
      next: (res: any) => {
        if (res.response) {
