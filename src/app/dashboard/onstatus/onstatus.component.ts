...
@Component({
  selector: 'app-onstatus',
  templateUrl: './onstatus.component.html',
  styleUrl: './onstatus.component.css'
})
export class OnstatusComponent {
 dataSource: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    this.service.dashbaordonoffstatu(this.merchantId,'ON').subscribe((res: any) => {
      if(res.flag==1)
      {
  ...
  excelexportCustomer() {
    ...
    let worksheet = workbook.addWorksheet('Today ON Status');
    // Blank Row
    worksheet.addRow([]);
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      FileSaver.saveAs(blob, 'Today ON Status.xlsx');
    });
  }
