...
@Component({
  selector: 'app-yesterdayoffstatus',
  templateUrl: './yesterdayoffstatus.component.html',
  styleUrl: './yesterdayoffstatus.component.css'
})
export class YesterdayoffstatusComponent {
dataSource: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    this.service.dashbaordyesterdayonoffstatu(this.merchantId,'OFF').subscribe((res: any) => {
      if(res.flag==1)
      {
  ...
  exportexcel() {
    ...
    this.onstatus?.forEach((element: any) => {
      this.response = [];
      let createdate = element.inActivatedDate;
  ...
  excelexportCustomer() {
    ...
    let worksheet = workbook.addWorksheet('Yesterday OFF Status');
    // Blank Row
    worksheet.addRow([]);
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      FileSaver.saveAs(blob, 'Yesterday OFF Status.xlsx');
    });
  }
