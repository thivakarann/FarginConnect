...
@Component({
  selector: 'app-dashboardviewfaulty',
  templateUrl: './dashboardviewfaulty.component.html',
  styleUrl: './dashboardviewfaulty.component.css'
})
export class DashboardviewfaultyComponent {
  dataSource: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    this.service.dashboardsetupboxStatusPagination(this.merchantId,2,this.pageSize,this.pageIndex).subscribe((res: any) => {
      if(res.flag==1)
      {
        this.faultystatus = res.response.content
        this.dataSource = new MatTableDataSource(this.faultystatus.reverse())
  ...
  exportexcel() {
    ...
    this.service.dashboardsetupboxStatusExport(this.merchantId,2).subscribe((res: any) => {
 ...
 if (res.flag == 1) {    
    ...
    this.faultystatus?.forEach((element: any) => {
      this.response = [];
...
excelexportCustomer() {
  ...
  let worksheet = workbook.addWorksheet('Damaged Setupbox');
  // Blank Row
  worksheet.addRow([]);
  ...
  workbook.xlsx.writeBuffer().then((data: any) => {
    ...
    FileSaver.saveAs(blob, 'Damaged Setupbox.xlsx');
  });
}
  ...
  customer(id:any){
    this.router.navigate([`dashboard/damaged-customer-view/${id}`], {
  ...
  bookedsearch(filterValue: string) {
    if (filterValue) {
    this.service.SetupboxDashboardSearch(this.merchantId,2,filterValue,this.pageSize1,this.pageIndex1).subscribe({
      next: (res: any) => {
        if (res.response) {
