...
@Component({
  selector: 'app-dashboardviewactiveempolyee',
  templateUrl: './dashboardviewactiveempolyee.component.html',
  styleUrl: './dashboardviewactiveempolyee.component.css'
})
export class DashboardviewactiveempolyeeComponent {
  dataSource: any;
...
searchPerformed: boolean=false;
  ...
  ngOnInit() {
    this.service.dashboardActiveInactiveCustomers(this.merchantid,1,this.pageSize,this.pageIndex).subscribe((res: any) => {
      if (res.flag == 1) {
  ...
  this.dataSource = new MatTableDataSource([]);
  this.dataSource = new MatTableDataSource(this.viewcustomer);
  this.dataSource.sort = this.sort;
  this.dataSource.paginator = this.paginator;
}
  ...
  exportexcel() {
    this.service.dashboardactiveinactiveexport(this.merchantid,1).subscribe((res: any) => {
      this.inactivecustomer = res.response;
      if (res.flag == 1) {
        let sno = 1;
  ...
  excelexportCustomer() {
    ...
    let worksheet = workbook.addWorksheet('Active customer');
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      FileSaver.saveAs(blob, 'Active Customer.xlsx');
    });
  }
  excelexportDetails() {
    const header = [
  ...
  customeronboard(filterValue: string) {
    ...
    this.service.CustomerActiveSearch(this.merchantid,1, filterValue,this.pageSize1,this.pageIndex1).subscribe({
      next: (res: any) => {
        if (res.response) {
