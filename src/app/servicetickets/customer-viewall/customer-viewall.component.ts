...
import { CustomerTicketraiseComponent } from '../customer-ticketraise/customer-ticketraise.component';
import { ViewDiscriptioncustomerComponent } from '../view-discriptioncustomer/view-discriptioncustomer.component';
...
import { ServicecustomerTicketImageComponent } from '../servicecustomer-ticket-image/servicecustomer-ticket-image.component';
...
searchPerformed: boolean=false;
  ...
  ngOnInit(): void {
    ...
    this.service.Customermerchanttickets(this.merchantId, this.pageSize, this.pageIndex).subscribe((res: any) => {
      if (res.flag == 1) {
        this.ticketmerchant = res.response?.content;
  ...
  exportexcel() {
    this.service.CustomermerchantticketsExport(this.merchantId).subscribe((res: any) => {
      this.ticketmerchantexport = res?.response;
 
      if (res.flag == 1) {
 
        let sno = 1;
        this.responseDataListnew = [];
        this.ticketmerchantexport.forEach((element: any) => {
          this.response = [];
  ...
  excelexportCustomer() {
    ...
    let worksheet = workbook.addWorksheet('Customer Request');
    // Blank Row
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      FileSaver.saveAs(blob, 'Customer Request.xlsx');
    });
  }
 
 
  customerticket(filterValue: string) {
    if (!filterValue) {
    ...
    this.service.CustomerTicketsSearch(this.merchantId,filterValue,this.pageSize1,this.pageIndex1).subscribe({
      next: (res: any) => {
        if (res.response) {
          this.ticketmerchant = res.response.content;  
  ...
  renderPage1(event: PageEvent) {
    ...
    // You can now fetch or display the data for the new page index
    // Example: this.fetchData(this.currentPageIndex, this.pageSize);
    this.customerticket(this.currentfilval);
  }
