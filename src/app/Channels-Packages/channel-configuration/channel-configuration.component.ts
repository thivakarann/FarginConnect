...
pageIndex1: number = 0;
  ...
  // }
   ...
   getData(event: any) {
    ...
    this.service.ChannelConfigurationSearch(this.currentfilval,event.pageSize,event.pageIndex).subscribe({
      next: (res: any) => {
        if (res.response) {
          this.Viewall = res.response;  
          this.dataSource = new MatTableDataSource(this.Viewall);  
      ...
      error: (err: any) => {
        ...
        this.service.channelconfiguration(event.pageSize,event.pageIndex).subscribe((res: any) => {
          if(res.flag==1)
          {
            this.channel = res.response.content;
            this.totalPages=res.pagination.totalElements;
