...
import { SuccessTransactionsComponent } from '../success-transactions/success-transactions.component';
import { FailureTransactionsComponent } from '../failure-transactions/failure-transactions.component';
...
export class OfflineTransactionsComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.roleName == 'Merchant Super admin') {
      this.valuestaticview = 'Static QR Transactions-View',
     this.valuestaticexport='Static QR Transactions-Export'
      ...
      this.service.viewRole(this.roleId).subscribe((res: any) => {
        ...
        if (res.flag == 1) {
          ...
          for (let datas of this.getdashboard) {
            ...
            if (this.actions == 'Static QR Transactions-View') {
              this.valuestaticview = 'Static QR Transactions-View'
            }
            if(this.actions=='Static QR Transactions-Export'){
              this.valuestaticexport='Static QR Transactions-Export'
             }
  ...
  viewsuccess(id: any, id1: any) {
    this.dialog.open(SuccessTransactionsComponent, {
  ...
  viewFailure(id: any, id1: any) {
    this.dialog.open(FailureTransactionsComponent, {
      enterAnimationDuration: "1000ms",
      exitAnimationDuration: "1000ms",
      disableClose: true,
      data: {
