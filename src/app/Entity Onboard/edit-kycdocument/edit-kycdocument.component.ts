...
export class EditKycdocumentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.KycForm = new FormGroup({
      ...
      gstNumber: new FormControl('', [
        Validators.required,
        Validators.pattern('/^[A-Z]{2}[0-9A-Z]{10}[A-Z0-9]{3}$/'),
      ]),
      drivingLicenseNumber: new FormControl('', [Validators.required]),
      cinch: new FormControl('', [
        Validators.required,
        Validators.pattern(
  ...
  getBackdocument(event: any) {
    ...
    if (
      this.imageFile1.type == 'image/png' ||
      this.imageFile1.type == 'image/jpeg' ||
      this.imageFile1.type == 'image/jpg'
    ) {
