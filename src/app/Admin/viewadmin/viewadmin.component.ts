...
import { Statusadmin, UnblockAdmin } from '../../fargin-model/fargin-model.module';
...
export class ViewadminComponent {
  dataSource: any;
  displayedColumns: string[] = ["sno","roletype", "role", "name", "mobilenumber", "email", "edit", "status", 
    "routestatus","View","blockStatus","unblock"]
...
searchPerformed: boolean=false;
  ...
  ngOnInit() {
    this.service.viewadminbyId(this.merchantId).subscribe((res: any) => {
      if (res.flag == 1) {
        this.admin = res.response;

        this.dataSource = new MatTableDataSource(this.admin.reverse());
    ...
    if (this.roleName == 'Merchant Super admin') {
      ...
      this.valueadminaccount='Employee-Unblock Account'
    }
    else {
      this.service.viewRole(this.roleId).subscribe((res: any) => {
        ...
        if (res.flag == 1) {
          ...
          for (let datas of this.getdashboard) {
            ...
            if(this.actions=='Employee-Unblock Account'){
              this.valueadminaccount='Employee-Unblock Account'
            }
          }
        }
  ...
  onSubmit(event: MatSlideToggleChange, id: any) {
    ...
    let submitModel: Statusadmin = {
      accountStatus: this.isChecked ? 1 : 0,
    };
    this.service.statusadmins(id, submitModel).subscribe((res: any) => {
  ...
  unblock(id:any){
    ...
    let submitModel:UnblockAdmin={
      merchantAdminId: id
    }
    this.service.UnblockAdmin(this.merchantId,submitModel).subscribe((res:any)=>{
      if(res.flag==1){
        this.toastr.success(res.responseMessage);
        setTimeout(() => {
          this.service.viewadminbyId(this.merchantId).subscribe((res: any) => {
