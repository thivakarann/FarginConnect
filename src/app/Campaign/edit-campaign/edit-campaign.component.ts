...
submit(): void {
  ...
  if (this.arrayExcel && this.arrayExcel.length > 0) {
      emailIds = this.arrayExcel.map((row: any) => row.emailAddress).filter((email: string) => !!email);
  }

  // Set emailIds to an empty array if no file is uploaded
  if (emailIds.length === 0) {
      this.toastr.warning('No file uploaded or no valid email addresses found. Proceeding with empty email addresses.');
