...
export class CreateCampaignsComponent {
  ...
  uploadBulkFile(event: Event): void {
    ...
    if (
      rejectedExtensions.includes(fileExtension) ||
      rejectedMimeTypes.includes(mimeType)
    ) {
      this.toastr.error(
        'File type not acceptable. Only CSV files are allowed!'
      );
    ...
    fileReader.onload = () => {
      ...
      // **Expected Headers**
      const expectedHeaders = ['emailAddress'].map((header) =>
        header.trim().toLowerCase()
      );
      ...
      const emailIds = this.arrayExcel
        .map((row: any) => row.emailAddress)
        .filter((email: any) => !!email); // Ensure 'emailAddress' exists
